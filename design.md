...
graph TB
    ...
    classDef process fill:#E3F2FD,stroke:#1565C0,stroke-width:2px,color:#0D47A1,font-weight:bold,rx:8px;
    classDef decision fill:#FFF3E0,stroke:#E65100,stroke-width:2px,color:#BF360C,font-weight:bold,rx:4px;
    classDef found fill:#E8F5E9,stroke:#2E7D32,stroke-width:2px,color:#1B5E20,rx:8px;
    classDef new fill:#FCE4EC,stroke:#C62828,stroke-width:2px,color:#B71C1C,font-weight:bold,rx:8px;

    %% ===== Nodes =====
    Word(("📝 Raw Word"))

    filter1{disabledProvider<br>Check Blocklist}
    filter2{DicProvider<br>Check Database}
    filter3{DefaultProvider<br>Create New Entry}

    DisabledWordEntry[("🚫 Ignored<br>(Proficiency = High)")]
    DatabaseWordEntry[("📚 Existing<br>(From Database)")]
    NewWordEntry[("✨ Brand New<br>(Need LLM Explain)")]

    %% ===== Edges =====
    Word --> filter1
    filter1 -->|"✅ Found"| DisabledWordEntry
    filter1 -->|"❌ Not Found"| filter2

    filter2 -->|"✅ Found"| DatabaseWordEntry
    filter2 -->|"❌ Not Found"| filter3

    filter3 -->|"✔️ Catch"| NewWordEntry

    %% ===== Apply Classes =====
    class Word process;
    class filter1,filter2,filter3 decision;
    class DisabledWordEntry,DatabaseWordEntry found;
    class NewWordEntry new;
```
...
The GetClient function will try every provider and return if it's available. You can modify the sequence on the `config.toml`
